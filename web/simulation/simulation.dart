...
void dessin(List l, num i, timer){
  if(i<l.length-2){
    l[i].delete();
    l[i+1].draw();
  }
...
voidRun(num vitesse, List parcour){
  num i=0;
      new Timer.periodic(new Duration(milliseconds:vitesse), (Timer timer) {  
        dessin(parcour,i++, timer);
      });
}
  ...
  Params.Resultats = new List<system>();   
    ...
    bouton_lancer.onClick.listen((Event e) { 
     ...
     //la sortie de la boucle ABC lorsque la Nième pièce est traitée
      ...
      void lancer(Evenement e, timer1){
       if ((e.Libelle_Evenement=='FSP') && (e.ServeurNo==1))
       {
         voidRun(vitesse,serv1ToSor());
         timer1.cancel();
       }          
       else if ((e.Libelle_Evenement=='FSP') && (e.ServeurNo==2))
       {
         voidRun(vitesse,serv2ToSor());
         timer1.cancel();
       }
       else if ((e.Libelle_Evenement=='RTP') && (e.ServeurNo==1))
       {
         voidRun(5,Ser1ToFile());
         timer1.cancel();
       }    
       else if ((e.Libelle_Evenement=='RTP') && (e.ServeurNo==2))
       {
         voidRun(5,serv2ToFil());
         timer1.cancel();
       }
       else if ((e.Libelle_Evenement=='FSP') && (e.ServeurNo==3))
       {
         voidRun(vitesse,serv2ToDes());
         timer1.cancel();              
       }
      }
      
      // lancement du traitement graphique
  
      /*
      Sys.EvenementsTraites.forEach((Evenement e) =>  new Timer.periodic(new Duration(seconds:2), (timer1) {  
        lancer(e, timer1);
      })       
      );*/
      
      /* L'utilisation des boucles pour parcourir tous les évenements fait que 
      le compilateur lance tout les Timer en même temps car il traite tous les 
      évenement en quelques millisecondes puis à la fin de la boucle les affiche en même temps.
      Vous pouvez le constater en enlevant les commentaires sur les quatre lignes de codes 806-809.
      c'est pourquoi nous avons choisi de lancer 11 timer différente pour pouvoir voir les évenements défiler 
      1 par 1.*/
      
      
      //Simulation des diffèrents cas
      List constructionCas(){
        var L1 = new List<Evenement>();
        var eve1 = new Evenement();                                                                          
        eve1.Libelle_Evenement = "FSP";
        eve1.ServeurNo = 2;
        L1.add(eve1);
        var eve2 = new Evenement();                                                                            
        eve2.Libelle_Evenement = "RTP";
        eve2.ServeurNo = 1;
        L1.add(eve2);
        var eve3 = new Evenement();                                                                            
        eve3.Libelle_Evenement = "FSP";
        eve3.ServeurNo = 2;
        L1.add(eve3);
        var eve4 = new Evenement();                                                                         
        eve4.Libelle_Evenement = "RTP";
        eve4.ServeurNo = 2;
        L1.add(eve4);
        var eve5 = new Evenement();                                                                          
        eve5.Libelle_Evenement = "FSP";
        eve5.ServeurNo = 1;
        L1.add(eve5);
        var eve6 = new Evenement();                                                                            
        eve6.Libelle_Evenement = "RTP";
        eve6.ServeurNo = 2;
        L1.add(eve6);
        var eve7 = new Evenement();                                                                            
        eve7.Libelle_Evenement = "RTP";
        eve7.ServeurNo = 1;
        L1.add(eve7);
        var eve8 = new Evenement();                                                                         
        eve8.Libelle_Evenement = "FSP";
        eve8.ServeurNo = 2;
        L1.add(eve3);
        var eve9 = new Evenement(); 
        eve9.Libelle_Evenement = "FSP";
        eve9.ServeurNo = 3;
        L1.add(eve9);
        var eve10 = new Evenement(); 
        eve10.Libelle_Evenement = "FSP";
        eve10.ServeurNo = 1;
        L1.add(eve10);
        var eve11 = new Evenement(); 
        eve11.Libelle_Evenement = "RTP";
        eve11.ServeurNo = 2;
        L1.add(eve11);
        return(L1);
      }
      ...
      void lancementCas(){ 
         List L1 = constructionCas();
        new Timer.periodic(new Duration(seconds:2), (timer1) {  
        lancer(L1[0], timer1);
      });
        new Timer.periodic(new Duration(seconds:7), (timer1) {  
        lancer(L1[1], timer1);
      });
        new Timer.periodic(new Duration(seconds:12), (timer1) {  
        lancer(L1[2], timer1);
      });
        new Timer.periodic(new Duration(seconds:14), (timer1) {  
        lancer(L1[3], timer1);
      }); 
        new Timer.periodic(new Duration(seconds:22), (timer1) {  
        lancer(L1[4], timer1);
      }); 
        new Timer.periodic(new Duration(seconds:27), (timer1) {  
        lancer(L1[5], timer1);
      }); 
        new Timer.periodic(new Duration(seconds:32), (timer1) {  
        lancer(L1[6], timer1);
      }); 
        new Timer.periodic(new Duration(seconds:37), (timer1) {  
        lancer(L1[7], timer1);
      }); 
        new Timer.periodic(new Duration(seconds:42), (timer1) {  
        lancer(L1[8], timer1);
      }); 
        new Timer.periodic(new Duration(seconds:47), (timer1) {  
        lancer(L1[9], timer1);
      });        
        new Timer.periodic(new Duration(seconds:52), (timer1) {  
        lancer(L1[10], timer1);
      });          
      }
